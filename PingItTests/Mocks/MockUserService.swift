...
final class MockUserService: UserServicing {
    ...
    private(set) var observeUserCallback: (@Sendable (User?) -> Void)?
    private(set) var removeUserListenerCalled = false

    func observeUser(
        id: String,
        onUpdate: @escaping @Sendable (User?) -> Void
    ) -> ListenerHandle {
        observeUserCallback = onUpdate
        return ListenerHandle { [weak self] in
            self?.removeUserListenerCalled = true
        }
    }
