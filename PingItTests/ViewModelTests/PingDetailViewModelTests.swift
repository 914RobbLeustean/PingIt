...
struct PingDetailViewModelTests {
    ...
    @Test func toggleRSVPSetsAttendingAndCount() async {
        let auth = authenticatedAuth(uid: "other-user")
        let ping = MockPingService()
        ping.rsvpPingResult = RSVPPingResult(rsvpCount: 3, isAttending: true)
        let vm = makeVM(authService: auth, pingService: ping)
        await vm.checkRSVPStatus()

        await vm.toggleRSVP()

        #expect(ping.rsvpPingCalled)
        #expect(vm.isAttending)
        #expect(vm.ping.attendingCount == 3)
    }

    @Test func toggleRSVPWithdrawal() async {
        let auth = authenticatedAuth(uid: "other-user")
        let ping = MockPingService()
        ping.hasUserRSVPdResult = true
        ping.rsvpPingResult = RSVPPingResult(rsvpCount: 0, isAttending: false)
        let vm = makeVM(authService: auth, pingService: ping)
        await vm.checkRSVPStatus()
        #expect(vm.isAttending)
        ...
        #expect(vm.isAttending == false)
        #expect(vm.ping.attendingCount == 0)
    }

    @Test func creatorCannotToggleRSVP() async {
        let auth = authenticatedAuth(uid: "creator1")
        let ping = MockPingService()
        let vm = makeVM(ping: makePing(creatorId: "creator1"), authService: auth, pingService: ping)
        await vm.checkRSVPStatus()

        await vm.toggleRSVP()

        #expect(ping.rsvpPingCalled == false)
        #expect(vm.canToggleRSVP == false)
    }

    @Test func rsvpDisabledWhileStatusCheckPending() {
        let auth = authenticatedAuth(uid: "other-user")
        let vm = makeVM(authService: auth)
        // checkRSVPStatus has not run yet
        #expect(vm.canToggleRSVP == false)
    }

    @Test func toggleRSVPSurfacesServiceError() async {
        let auth = authenticatedAuth(uid: "other-user")
        let ping = MockPingService()
        ping.errorToThrow = PingItError.firestoreWriteFailed(underlying: URLError(.notConnectedToInternet))
        let vm = makeVM(authService: auth, pingService: ping)
        ping.errorToThrow = nil
        await vm.checkRSVPStatus()
        ping.errorToThrow = PingItError.firestoreWriteFailed(underlying: URLError(.notConnectedToInternet))
