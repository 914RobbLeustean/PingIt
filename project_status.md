...
## Completed
- 2026-06-01 — Stability fixes: ChatView crash on empty chats (Date.distantPast → Firebase Timestamp overflow) now falls back to Date.now. ChatView.onDisappear's leaveChat call runs as a background detached task with a 5-second TaskGroup timeout to prevent watchdog SIGKILL on app termination. ChatBubbleShape guards against zero-sized rects. PingDetailView's ping-unavailable auto-dismiss is skipped while ChatView is pushed on top so it doesn't pop both screens. Onboarding loop fixed: errors now surface to the UI with a "Continue Anyway" pill; OnboardingViewModel writes via mergeUser (setData merge: true) instead of updateUser. New UserService.ensureUserProfileExists backfills a placeholder user doc + username reservation when an Auth user has no Firestore doc (recovers from orphan accounts created by partially-failed signups). RootView falls through to the backfill on fetch failure so any broken account self-heals on next launch. Feed avatars now load profileImageUrl via AsyncImage instead of always showing the initial letter.
- 2026-06-01 — UI overhaul (audit sweep): redesigned the last screens still using default iOS styling. `SuspendedAccountView` (dark hero w/ pingHot rings + Syne title + surface-pill Sign Out), `PasswordStrengthView` (pingHot/pingAccent/pingLive segments + DM Sans rules). Replaced the two `Ping Unavailable` system `.alert()` dialogs with auto-dismissing `MapAlertChip` toasts (3s on Map, 2s on PingDetail before it pops back).
- 2026-06-01 — UI overhaul (Chat reactions + location bubble): replaced the system `.contextMenu` on chat bubbles with a custom `MessageActionOverlay` — long-press fires a medium haptic and shows a translucent overlay with an amber-bordered reaction pill (8 emoji bouncing in with a stagger) plus a stacked Report/Block action card. ChatLocationBubble rewritten so the Map fills the `ChatBubbleShape` edge-to-edge with the name floating on a dark gradient overlay; mini-preview matches main Map style (POIs hidden, flat elevation).
