...
final class PingService: PingServicing {
    ...
    func rsvpPing(pingId: String) async throws -> RSVPPingResult {
        do {
            let result = try await functions.httpsCallable("rsvpPing").call(["pingId": pingId])
            guard
                let data = result.data as? [String: Any],
                let rsvpCount = Self.intValue(data["rsvpCount"]),
                let isAttending = Self.boolValue(data["isAttending"])
            else {
                throw PingItError.firestoreReadFailed(underlying: PingItError.documentNotFound)
            }

            return RSVPPingResult(rsvpCount: rsvpCount, isAttending: isAttending)
        } catch let error as PingItError {
            throw error
        } catch {
            throw PingItError.firestoreWriteFailed(underlying: error)
    ...
    func hasUserRSVPdPing(pingId: String, userId: String) async throws -> Bool {
        let document = try await db
            .collection(Constants.Firestore.rsvpsCollection)
            .document("\(pingId)_\(userId)")
            .getDocument()
    ...
    func fetchPings(createdAfter date: Date) async throws -> [Ping] {
        do {
            let snapshot = try await db.collection(Constants.Firestore.pingsCollection)
                .whereField("createdAt", isGreaterThan: date)
                .getDocuments()
            return snapshot.documents.compactMap { try? $0.data(as: Ping.self) }
        } catch {
            throw PingItError.firestoreReadFailed(underlying: error)
